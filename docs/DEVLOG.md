...
### Summary
Established the runtime foundation for Phase 6 (Mentoría 1-a-1 subscription). Added Zod-validated env access (`src/lib/env.ts`), Drizzle ORM with Neon serverless driver, the `products` table schema with `product_kind` enum, generated and applied the initial migration (with `citext` and `pgcrypto` extensions), seeded the `mentoria-1a1` product row, configured Vitest for integration tests with a passing smoke test against the live DB, and added a `/api/health` endpoint that proves end-to-end DB connectivity. Build verified via `next build` directly (see caveat 6).

### What Was Built
- `src/lib/env.ts` — Zod schema validating `DATABASE_URL`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID_MENTORIA`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `APP_URL`, optional `ADMIN_SEED_EMAIL`. Fail-fast at module load.
- `drizzle.config.ts`, `src/db/client.ts`, `src/db/schema.ts` — Drizzle wired to Neon serverless. `products` table with `id` (uuid), `kind` (enum), `slug` (unique), `name`, `priceMxn`, `currency`, `capacity`, `stripePriceId`, `stripeProductId`, `metadata` (jsonb), `createdAt`.
- `src/db/migrations/0000_init.sql` — generated by drizzle-kit, manually prepended with `CREATE EXTENSION IF NOT EXISTS citext;` and `CREATE EXTENSION IF NOT EXISTS pgcrypto;` per plan.
- `package.json` scripts — `db:migrate`, `db:generate`, `db:seed`, `prebuild` (runs `db:migrate`), `test`, `test:watch`.
- `scripts/seed-products.ts` — Idempotent seed for `mentoria-1a1` row at MXN 2222, capacity 8. `stripeProductId` left as `prod_placeholder` (human will replace).
- `vitest.config.ts` + `tests/integration/setup.ts` + `tests/integration/products.test.ts` — Integration test scaffold (`fileParallelism: false`, `testTimeout: 30s`); products smoke test passes (1/1).
- `src/app/api/health/route.ts` — Executes `SELECT 1` via Drizzle, returns `{ status: 'ok' }`.

### Verification Results
- `npx tsx --env-file=.env.local -e "import('./src/lib/env.ts').then(m => console.log(Object.keys(m.default.env)))"` — prints all 7 required keys (`ADMIN_SEED_EMAIL` is optional, absent).
- Migration applied to Neon successfully. `information_schema.columns` shows all 11 columns with expected types.
- Seed inserted 1 row; verified via Drizzle select.
- `vitest run products.test.ts` — 1 passed.
- `next build` — compiled successfully, `/api/health` registered as dynamic route.

### Known caveats

1. `tsx` does not auto-load `.env.local`. Workaround: `npx tsx --env-file=.env.local ...`. Plan's verification command (Task 1.2 Step 3) needs a doc update.
2. `psql` is not installed in the local dev environment. Verifications use Drizzle JS queries instead.
3. `npm audit` reports 17 vulnerabilities (12 moderate, 5 high) after installing Phase 6 deps. Out of scope for S1.
4. Node v23 emits EBADENGINE warning for `eslint-visitor-keys@5.0.1`. Non-blocking.
5. `.env.example` was added to the repo (commit c6fadb2) but not specified in the plan. Plan Task 1.2 should include it as an output. The plan needs a doc fix.
6. `npm run build` fails locally because `prebuild` invokes `drizzle-kit migrate`, which does not auto-load `.env.local` even though Next.js itself does. On Vercel this is fine because env vars are set in the platform. Local boot verification used `./node_modules/.bin/next build` directly. Also: when using `node --env-file=.env.local next build`, Next.js workers reject the `NODE_OPTIONS` value (`ERR_WORKER_INVALID_EXEC_ARGV`).
